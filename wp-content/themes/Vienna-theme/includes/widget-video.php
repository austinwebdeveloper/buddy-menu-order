...
class pulsar_video_widgets extends WP_Widget {
	function pulsar_video_widgets() {
		...
		$widget_ops = array('classname' => 'pulsar-videos','description' => esc_html__('Video Widget - supports Youtube, Vimeo, Dailymotion',TEXT_DOMAIN));
		/* $control_ops = array( 'twitter name' => 'pulsar', 'count' => 3, 'avatar_size' => '32' ); */		
		parent::__construct('pulsar-videos',esc_html__('[Micro Themes] - Videos',TEXT_DOMAIN),$widget_ops);
...
function form( $instance ) {
		...
		$defaults = array( 'title' => esc_html__('Videos',TEXT_DOMAIN), 
			'title' => 'Popular Video', 
		...
		<input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" value="<?php echo esc_attr($title); ?>"  class="widefat" />
		</p>
        ...
        <p>
        <label for="<?php echo $this->get_field_id( 'type' ); ?>"><?php esc_html_e('Video Type:', TEXT_DOMAIN) ?></label>
        <select id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo esc_attr($this->get_field_name( 'type' )); ?>" class="widefat">
            <option <?php if ( 'Youtube' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_html_e('Youtube', TEXT_DOMAIN) ?></option>
            <option <?php if ( 'Vimeo' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_html_e('Vimeo', TEXT_DOMAIN) ?></option>
            <option <?php if ( 'Dailymotion' == $instance['type'] ) echo 'selected="selected"'; ?>><?php esc_html_e('Dailymotion', TEXT_DOMAIN) ?></option>
        </select>
        </p>

		<p>
		<label for="<?php echo $this->get_field_id( 'id' ); ?>"><?php esc_html_e('Video ID:', TEXT_DOMAIN) ?></label>
		<input id="<?php echo $this->get_field_id( 'id' ); ?>" name="<?php echo esc_attr($this->get_field_name( 'id' )); ?>" value="<?php echo esc_attr($id); ?>" class="widefat" />
		</p>
