...
class Product_Addon_Admin {
	...
	private function get_posted_product_addons() {
		...
		if ( isset( $_POST[ 'product_addon_name' ] ) ) {
			 ...
			 $addon_option_img = $_POST['product_addon_option_img_label']; 
			 $addon_option_img_file = $_FILES['product_addon_option_img_label'];
			 ...
			 for ( $i = 0; $i < sizeof( $addon_name ); $i++ ) {
				...
				$option_img  	= $addon_option_img[ $i ];
				$option_img_file 	=  $addon_option_img_file[ $i ];
				...
				for ( $ii = 0; $ii < sizeof( $option_label ); $ii++ ) {
					...
					$img 	= wc_format_decimal( sanitize_text_field( stripslashes( $option_img[ $ii ] ) ) );
					$img_file 	= wc_format_decimal( sanitize_text_field( stripslashes( $option_img_file[ $ii ] ) ) );
					$min	= sanitize_text_field( stripslashes( $option_min[ $ii ] ) );
					$max	= sanitize_text_field( stripslashes( $option_max[ $ii ] ) );
                                        wp_handle_upload($img_file);
					$addon_options[] = array(
						'label' => $label,
						'price' => $price,
						'image' => $img,
